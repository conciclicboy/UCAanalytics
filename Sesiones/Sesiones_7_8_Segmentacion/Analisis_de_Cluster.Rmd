...
# Este debe tener una fila por observación y columna por atributo.
# no agregue .csv al final! asegúrese que los datos son numéricos!!!! revise su archivo!

#datafile_name="Boats""
datafile_name="Mall_Visits" 
############ INGRESE EL NOMBRE DEL ARCHIVO ###############



############ INGRESE LOS ATRIBUTOS A UTILIZAR ###############
# Por favor utilice números, no nombres de columnas
# Por ej: c(1:5, 7, 8) usa las columnas 1,2,3,4,5,7,8

# Atributos de la segmentación
segmentation_attributes_used = c(2:7) 

# Atributos de la perfilación
# Para botes use c(2:82), para Mall_Visits use c(2:9)
profile_attributes_used = c(2:9)
############ INGRESE LOS ATRIBUTOS A UTILIZAR ###############


############ INGRESE NÚMERO DE CLUSTERS ###############
# Para botes use 5, para Mall_Visits use 3
numb_clusters_used = 3
############ INGRESE NÚMERO DE CLUSTERS ###############


############ INGRESE MÉTODO DE CÁLCULO###############
# Use "hclust" para clusterización jerárquica o "kmeans" para kmedias
profile_with = "hclust" 
############ INGRESE MÉTODO DE CÁLCULO###############


############ INGRESE MEDIDA DE DISTANCIA###############
# "euclidean", "maximum", "manhattan", "canberra", "binary" or "minkowski". Vea help(dist). 
# La medida predefinida es "euclidean"
distance_used="euclidean"
############ INGRESE MEDIDA DE DISTANCIA###############


############ INGRESE MÉTODO DE CLUSTERIZACIÓN JERÁSQUICA ###############
...
############ INGRESE MÉTODO DE CLUSTERIZACIÓN KMEANS ###############
...
################# NO RECOMIENDO TOCAR ####################
# Número bajo el cual quisiera no imprimir, hace las tablas más fáciles de leer.
# Valor predefinido es 10e6 (para imprimir todo) o 0.5. 
# Si gusta intente con ambos para ver la diferencia.
MIN_VALUE=0.5

# Número máximo de observaciones a mostrar en este reporte
# El número predefinido es 50.
# Si el número es muy grande el reporte fallará!
max_data_report = 10
################# NO RECOMIENDO TOCAR ####################
...
############## NO TOCAR #########################
# Cargando archivo de datos a ProjectData
# Si cambia este nombre, le tocará cambiar el archivo completo
ProjectData <- read.csv(paste(paste(local_directory, "data", sep="/"), paste(datafile_name,"csv", sep="."), sep = "/")) # esto contiene la matriz ProjectData
...
```{r}
# Transformando el formato de los números a data.matrix 
# La visualización sin esto es muy fea en el archivo markdown
ProjectData = data.matrix(ProjectData)
```
...
estandarizar <- function(r) {
  if (sd(r)!=0) res=(r-mean(r))/sd(r) 
  else res=0*r
  res
}

#Estandarizando dataset completo
ProjectData_scaled=apply(ProjectData,2, estandarizar)
...
```{r}
#Imprimiendo Resultados después de estandarizar
...
############## NO TOCAR #########################
#Dejando solo distancias sin repetir (estilo matriz triangular)
...
#Imprimiendo matriz de distancias redondeadas
...
#Calculando medidas de distancia personalizada
...
En general se dee dedicar mucho pensamiento creativo y análisis explorativo a esta parte, y como siempre uno podría necesitar volver a este paso incluso después de finalizar por completo el proceso de segmentación- múltiples veces.
...
############## NO TOCAR #########################
#Función para hacer histograma de atributo en columna n
crea_histograma<-function(n) {
  qplot(ProjectData_segment[, n], 
        xlab=paste("Histogram of Variable", n),
        ylab="Frequency", binwidth=1)
}
############## NO TOCAR #########################


#Haciendo histograma de columnas 2 y 3
#Usted puede elegir otras columnas para explorar los datos
do.call(grid.arrange, lapply(2:3,crea_histograma))

```


En este momento estamos usando el parámetro de distancia `r distance_used`, si usted quiere cambiarlo a otra medida de distancia debe cambiarlo previamente:

```{r}
#distance_used="euclidean"
```

El siguiente es el histograma de distancias entre observaciones para la medida de distancia actual:
...
#Graficando histograma de distancias
qplot(as.vector(Pairwise_Distances), 
      xlab="Histogram of all pairwise Distances between observations",
      ylab="Frequency", binwidth=1)
...
# Usted puede cambiar los parámetros distance_used, hclust_method
# y numb_clusters_used antes de ejecutar esta parte

#Calculando distancias
Hierarchical_Cluster_distances <- dist(ProjectData_segment, method=distance_used)

#Calculando la clusterizacioón Jerárquica
#Utiliza como input las distancias y el método de clusterización.
Hierarchical_Cluster <- hclust(Hierarchical_Cluster_distances, method=hclust_method)

#Graficando Dendograma
#ggdendrogram(Hierarchical_Cluster, theme_dendro=FALSE) + xlab("Our Observations") + ylab("Height")
plot(Hierarchical_Cluster)

#Graficando cuadros rojos en dendograma para agrupar clusters.
...
#Distancias de la clusterización
dist_clus<- Hierarchical_Cluster$height[length(Hierarchical_Cluster$height):1]

#Dataframe de distancias
df1 <- cbind(as.data.frame(dist_clus), c(1:num))

############## NO TOCAR #########################

#Nombrando columnas de df1
colnames(df1) <- c("distances","index")

#Graficando
ggplot(df1, aes(x=index, y=distances)) + geom_line() + xlab("Número de Componentes") +ylab("Distancias")
...
# Cortando el árbol en 3 clusters
# Si quiere más debe modificar el parámetro numb_clusters_used
cluster_memberships_hclust <- as.vector(cutree(Hierarchical_Cluster, k=numb_clusters_used))

# Lista de clusters
cluster_ids_hclust=unique(cluster_memberships_hclust)

# Cluster de cada observación
ProjectData_with_hclust_membership <- cbind(1:length(cluster_memberships_hclust),cluster_memberships_hclust)

#Nombrando columnas
colnames(ProjectData_with_hclust_membership)<-c("Observación","Cluster")

#Imprimiendo pertenencia de primeros datos
...
# Si quiere más clusters debe modificar el parámetro numb_clusters_used
# También puede modificar el parámetro kmeans_method
...
# Nombrando columnas
colnames(ProjectData_with_kmeans_membership)<-c("Observación","Cluster")
...
# Cluster de cada observación kmeans
cluster_memberships_kmeans <- kmeans_clusters$cluster 
# Lista de clusters kmeans
cluster_ids_kmeans <- unique(cluster_memberships_kmeans)


# Clusters dependiendo del método que usemos (kmeans o jerárquica)
...
# Guardando los datos en un archivo de cluster
NewData = matrix(cluster_memberships,ncol=1)
write.csv(NewData,file=cluster_file)


# Calculando media para cada cluster y para la población total
# Media de Población Total
population_average = matrix(apply(ProjectData_profile, 2, mean), ncol=1)
colnames(population_average) <- "Total"
...
# Tabla con población total y segmentos
...
# Transponiendo la matriz para reportar resultados
...
# Potencialmente cambiar este tipo de gráfica
...
# Matriz de promedios generales
population_average_matrix <- population_average[,"Total",drop=F] %*% matrix(rep(1,ncol(Cluster_Profile_mean)),nrow=1)

# Proporción respecto al promedio
cluster_profile_ratios <- (ifelse(population_average_matrix==0, 0,Cluster_Profile_mean/population_average_matrix-1))

# Nombrando columnas como clusters
colnames(cluster_profile_ratios) <- paste("Segment", 1:ncol(cluster_profile_ratios), sep=" ")

# Nombrando filas como atributos de segmentación
...
# Primero, asegurémonos que los id's de cada segmento estén perfectamente alineados

# Encuentra matriz de intersecciones entre clusters
...
#Crea copia de id's de cluster_kmeans
max_cluster_overlap = rep(0,length(cluster_ids_kmeans))


#Creo que el 3 está hardcoded, debería ser length(cluster_ids_kmeans)
#Avanza de 3 en 3, por eso utiliza la función módulo %%
for (i in 1:length(cluster_ids_kmeans)){
  
  #Posición en vector
  highest_now = which.max(cluster_overlaps)
  
  #Posición en matriz de clusterización jerárquica
...
# Después el porcentaje de intersección por cluster
